...
class ENV_dqn(object):
    ...
    def step(self, actions, eval_set1=set(), eval_set2=set()):
        # if actions[0] == 0:
        #     self.world.agents[0].phi+=0.1
        # if actions[0] == 2:
        #     self.world.agents[0].phi-=0.1
        # if actions[1] == 0:
        #     self.world.agents[1].phi+=0.1
        # if actions[1] == 2:
        #     self.world.agents[1].phi-=0.1
        # if actions[2] == 0:
        #     self.world.agents[2].phi+=0.1
        # if actions[2] == 2:
        #     self.world.agents[2].phi-=0.1
        done = False
        temp_metric = []
        ...
        while self.world.steps < (1 + self.steps) * self.interval:
            self.world.step(actions[0])
            self.world.step(actions[0])
            temp_r = [self.sf.reward(self.world.agents[i], self.world) for i in range(1)]
            temp_metric.append([self.sf.metrics(self.world.agents[i], self.world) for i in range(1)])
        self.steps += 1
        if self.steps == self.length:
            done = True
